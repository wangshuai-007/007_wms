...
namespace DevExpress.Win.FunctionalTests {
	...
	public class RadioGroupTests {
		...
		public void SelectRadioItemViaMouseTest() {
			using(new EditorsTestInitializer()) {
				this.UIMap.SwitchToRadioEditDemoModule();
				this.UIMap.SelectRadioItemViaMouse();
				this.UIMap.CheckSelectedStateTrue();
			}
		}
		[Timeout(TestInitializer.timeOut), TestCategory("WorkOnFarm"), TestCategory("GridEditorsNavBar"), TestCategory("VS11"), TestMethod]
		public void SelectRadioItemViaKeyboardTest() {
			using(new EditorsTestInitializer()) {
				this.UIMap.SwitchToRadioEditDemoModule();
				this.UIMap.SelectRadioItemViaKeyboard();
				this.UIMap.CheckSelectedStateTrue();
			}
		}
		[Timeout(TestInitializer.timeOut), TestCategory("WorkOnFarm"), TestCategory("GridEditorsNavBar"), TestCategory("VS11"), TestMethod]
		public void SelectRadioItemViaMouseWheelScrollTest() {
			using(new EditorsTestInitializer()) {
				this.UIMap.SwitchToRadioEditDemoModule();
				this.UIMap.SelectRadioItemViaMouseWheelScroll();
				this.UIMap.CheckSelectedStateTrue();
			}
		}
